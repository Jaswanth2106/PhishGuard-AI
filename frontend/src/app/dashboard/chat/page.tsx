...
export default function ChatPage() {
  ...
  useEffect(() => {
    // Only save if there are actual messages (or clear it if empty)
      ...
      <div className="flex-1 glass-card rounded-xl border border-border/50 flex flex-col overflow-hidden relative shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          {messages.length === 0 && !isLoading ? (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-4 animate-in fade-in zoom-in duration-500 pb-12">
              <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary mb-2 shadow-inner">
                <ShieldAlert className="w-8 h-8" />
              </div>
              <h3 className="text-2xl font-bold tracking-tight">Security Assistant</h3>
              <p className="text-muted-foreground max-w-sm text-sm leading-relaxed">
                Ask questions about phishing, cybersecurity, malicious links, spoofing, email analysis, or security best practices.
              </p>
              <div className="pt-6 flex flex-col items-center gap-3 text-sm text-muted-foreground/70">
                <span className="bg-muted/50 px-4 py-1.5 rounded-full border border-border/50 font-medium">No conversation yet.</span>
